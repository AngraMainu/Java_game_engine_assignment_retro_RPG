...
public class ObjectBoots extends Object {
    ...
    public ObjectBoots(PlayPanel pp) {
        super(pp);
        name = "Boots";
        try{
            image = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/Objects/Objects prototype/boots.png")));
            image = util.scaleImage(image, pp.tileSize, pp.tileSize);
        } catch (IOException e){
            LOGGER.severe("Couldn't find path of the image!");
        }
        collision = true;
        this.description = "To prepare for what's ahead...";
    }
